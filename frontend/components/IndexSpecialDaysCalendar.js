...
const CalendarScreen = () => {
	const { myFamilyMembers, userId, familyId } = useCommon();
	...
	const fetchEvents = async () => {
		try {
			const response = await fetch(
				`http://10.0.2.2:3000/special-days/${familyId}`
			);
	...
	const handleJoinEvent = async () => {
		...
		try {
			...
			const response = await fetch(
				`http://10.0.2.2:3000/special-days/update/${eventDetails[selectedDate]._id}`,
				{
					method: 'PUT',
		...
		<ScrollView
			...
			{selectedDate && eventDetails[selectedDate] ? (
				<View style={styles.eventContainer}>
					...
					<Text style={styles.eventText}>📅 Ngày diễn ra: {selectedDate}</Text>
